...
def map_json(request):
    ...
    featureset = Recording.objects.all().order_by('-date').exclude(location__isnull=True).exclude(location__exact='')[:750]
...
def map_location_json(request, ne_lat=0, ne_lon=0, sw_lat=0, sw_lon=0):
    ...
    featureset = Recording.objects.order_by('-date')[:750]
